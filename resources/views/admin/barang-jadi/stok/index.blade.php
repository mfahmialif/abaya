...
@section('title', 'Stok Barang Jadi')
@section('content')
    ...
    @include('admin.barang-jadi.stok.edit')
@endsection
...
@push('scripts')
    <script>
        $(document).on('submit', '.form-delete-record', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.barang-jadi.stok.delete') }}",
        ...
        var dataTable = initDataTables('table-1', 'loader-user', 'card1', null, false,
            'Stok Barang Jadi', "{{ route('admin.barang-jadi.stok.data') }}",
            [{
                    data: "kode",
                    name: "kode",
