...
<div class="offcanvas offcanvas-end" id="edit-record">
    ...
    <div class="offcanvas-body flex-grow-1">
        <form class="record pt-0 row g-2" id="form-edit-record" action="{{ route('admin.barang-jadi.keluar.update') }}"
            method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <input type="hidden" name="id">
            @include('admin.barang-jadi.keluar.form')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit-record-button', function() {
                ...
                $('#form-edit-record [name="jumlah"]').attr('disabled', false);
                $('#form-edit-record [name="tanggal"]').attr('disabled', false);
                $('#form-edit-record [name="keterangan"]').attr('disabled', false);
