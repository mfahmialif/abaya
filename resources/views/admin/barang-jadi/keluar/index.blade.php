...
@section('title', 'Barang Jadi Keluar')
@section('content')

    @include('admin.barang-jadi.keluar.filter')
    ...
    @include('admin.barang-jadi.keluar.add')
    @include('admin.barang-jadi.keluar.edit')
...
@push('scripts')
        ...
        $(document).on('submit', '.form-delete-record', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('admin.barang-jadi.keluar.delete') }}",
    ...
    <script>
        var dataTable = initDataTables('table-1', 'loader-user', 'card1', 'new-record-button', false,
            'Barang Jadi Keluar', "{{ route('admin.barang-jadi.keluar.data') }}",
            [{
                    data: "nama",
                    name: "nama",
        ...
        function initSelectUkuranBarang(selectElement, formId) {
            $(formId).find('[name="jumlah"]').attr('disabled', true);
            $(formId).find('[name="tanggal"]').attr('disabled', true);
            $(formId).find('[name="keterangan"]').attr('disabled', true);
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $(formId).find('[name="jumlah"]').attr('disabled', false);
                    $(formId).find('[name="tanggal"]').attr('disabled', false);
                    $(formId).find('[name="keterangan"]').attr('disabled', false);
